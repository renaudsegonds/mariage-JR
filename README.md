...
Une seule page (`index.html`), pensée pour être ouverte depuis le QR code à la fin de la vidéo faire-part : les invités y retrouvent la date, le lieu (avec boutons GPS) et les infos pratiques.
...
- [ ] **Les coordonnées GPS** dans les 3 boutons (Google Maps, Waze, Plans) : remplacer `43.6412,-1.4462` par les vraies coordonnées ; clic droit sur le lieu dans Google Maps → le premier élément du menu copie les coordonnées
...
- Aucune dépendance, aucun build : un seul fichier HTML autonome (les polices viennent de Google Fonts).
